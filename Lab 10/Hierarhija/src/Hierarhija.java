...
class Graph<E> {
    ...
    public int findIndex(String name) {
        for(int i = 0;i<adjList.length;i++){
            if(adjList[i].getInfo().equals(name)) {
                return i;
            }
        }
        return -1;
...
public class Hierarhija {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0;i<m;i++){
            ...
            String ime1 = parts[1];
            String ime2 = parts[3];
            int tezina = Integer.parseInt(parts[4]);
            g.adjList[x].setInfo(ime1);
            g.adjList[y].setInfo(ime2);
            g.addEdge(x, y, tezina);
        }
        int start = g.findIndex(br.readLine());
        List<Edge> mst = g.prim(start);
        int sum = 0;
