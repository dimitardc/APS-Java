...
public class Competition {
    ...
    public static void competition(SLL<Competitor> prevYearCompetitors, SLL<Competitor> currYearCompetitors) {
        SLL<Competitor> res = new SLL<>();
        SLLNode<Competitor> node = prevYearCompetitors.getFirst();
        double sum = 0;
        while(node!=null){
            sum += node.element.getPoints();
            node = node.succ;
        }
        double sumPoints = sum/prevYearCompetitors.length();

        node = currYearCompetitors.getFirst();
        while(node!= null){
            if(node.element.getPoints() < sumPoints){
                currYearCompetitors.delete(node);
            }
            node = node.succ;
        }
    }
