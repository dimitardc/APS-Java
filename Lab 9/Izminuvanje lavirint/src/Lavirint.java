    ...
    void findPath(){
        ...
        if(s.isEmpty())
            ...
            while(!s.isEmpty())
                os.push(s.pop());
            while(!os.isEmpty())
                System.out.println(rehash.get(os.pop()));
        }
    }
...
public class Lavirint {
    public static void main(String[] args) throws IOException {
        ...
        int rows, collums;
        String rowscollums[] = br.readLine().split(",");
        rows = Integer.parseInt(rowscollums[0]);
        collums = Integer.parseInt(rowscollums[1]);
        String parts[] = new String[rows];
        for(int i = 0;i<rows;i++){
            parts[i] = br.readLine();
        }
        Maze m = new Maze();
        m.generateGraph(rows,collums,parts);
        m.findPath();
    }
}